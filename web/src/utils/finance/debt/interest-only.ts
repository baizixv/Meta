...
// 获取每期还款额度
export const getInterestOnlyMonthPay = (
  A: number, // 借贷本金
  N: number, // 借贷期数
  R: number // 每期利率
): number => {
...
// 获取先息后本的利率,二分法逼近进行近似计算
export const getInterestOnlyRate = ({
  debtMoney,
  debtCount,
  debtTerm,
}: DebtParamsSencond) => {
  ...
  const guessResult = binarySearch(
    ...
    (guess: number) => {
      const monthlyPayGuess = getInterestOnlyMonthPay(
        debtMoney,
        debtTerm,
        guess
      )
...
// 获取先息后本月供账单
export const getInterestOnlyMonthPayArray = ({
  debtMoney,
  debtRate,
  debtTerm,
  ...
  const R = debtRate
  // 每期偿还总额
  const monthMoney = getInterestOnlyMonthPay(debtMoney, debtTerm, R)
